...
public class GeneratorScreen extends AbstractContainerScreen<GeneratorMenu> {
    private static final ResourceLocation TEXTURE =
            new ResourceLocation(InfinityNexusMod.MOD_ID, "textures/gui/generator_gui.png");
    ...
    private void renderTooltips(GuiGraphics pGuiGraphics, int pMouseX, int pMouseY, int x, int y) {
        if (Screen.hasShiftDown()) {
            ...
            }else if(isMouseAboveArea(pMouseX, pMouseY, x, y, 79, 46, 17, 17)) {
    ...
    private void renderProgressArrow(GuiGraphics guiGraphics, int x, int y) {
        if(menu.isCrafting()) {
            guiGraphics.blit(TEXTURE, x + 94, y + 44, 189,13, -13, -13 +menu.getScaledProgress());

            //guiGraphics.blit(TEXTURE, x + 120, y + 68 , 238, 62,  -menu.getScaledProgress(), -62);

            //guiGraphics.blit(TEXTURE, x + 95, y + 45, textureWidth - scaledProgress, 0, scaledProgress, textureHeight);
