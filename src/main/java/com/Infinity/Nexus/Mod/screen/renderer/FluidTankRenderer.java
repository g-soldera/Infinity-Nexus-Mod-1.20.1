    ...
    public List<Component> getTooltip(FluidStack fluidStack, TooltipFlag tooltipFlag) {
        ...
        try {
            ...
            if (tooltipMode == TooltipMode.SHOW_AMOUNT_AND_CAPACITY) {
                MutableComponent amountString = Component.translatable("infinity_nexus_mod.tooltip.liquid.amount.with.capacity", nf.format(milliBuckets), nf.format(capacity));
                tooltip.add(amountString.withStyle(ChatFormatting.GRAY));
            } else if (tooltipMode == TooltipMode.SHOW_AMOUNT) {
                MutableComponent amountString = Component.translatable("infinity_nexus_mod.tooltip.liquid.amount", nf.format(milliBuckets));
                tooltip.add(amountString.withStyle(ChatFormatting.GRAY));
            }
        } catch (RuntimeException e) {
            InfinityNexusMod.LOGGER.error("Failed to get tooltip for fluid: " + e);
        }
